...
import { FaUsers, FaClipboardList, FaFileExcel, FaSignOutAlt, FaEdit, FaTrash, FaFilter, FaTimes, FaPlus, FaFileImport, FaHome, FaUserCircle, FaUser, FaClipboard } from 'react-icons/fa';
...
const AdminDashboard = ({ setRole }) => {
  ...
  const filteredSureties = sureties.filter(surety =>
    (surety.shurityName?.toLowerCase().includes(filter.toLowerCase())) ||
    (surety.caseFirNo?.toLowerCase().includes(filter.toLowerCase()))
  );
      ...
      <div className="flex-1 p-8 sm:p-10">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xl">
          ...
          <div className="mb-6 relative">
            <input
              type="text"
              placeholder={view === 'users' ? "Filter by mobile number, DOB, name, or email..." : "Filter by surety name or case/FIR no..."}
              value={filter}
